 ...
 * @author viviano
 */
public class BuscaFreiraAtivasEnclaustradas implements Command{
    ...
    public String execute(HttpServletRequest request) {
        ...
        try {
            ...
            for(FreiraBean f : aux){
                if(f.getTipoAtivo().equals("enclaustrada")){
                freiras.add(f);
                }
            }
            
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(BuscaFreiraAtivasEnclaustradas.class.getName()).log(Level.SEVERE, null, ex);
