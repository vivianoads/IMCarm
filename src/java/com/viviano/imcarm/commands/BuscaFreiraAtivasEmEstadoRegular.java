 ...
 * @author viviano
 */
public class BuscaFreiraAtivasEmEstadoRegular implements Command{
    ...
    public String execute(HttpServletRequest request) {
        ...
        try {
            ...
            for(FreiraBean f : aux){
                if(f.getTipoAtivo().equals("regular")){
                freiras.add(f);
                }
            }
            
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(BuscaFreiraAtivasEmEstadoRegular.class.getName()).log(Level.SEVERE, null, ex);
