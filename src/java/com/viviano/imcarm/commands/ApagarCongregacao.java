/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */

package com.viviano.imcarm.commands;

import com.viviano.imcarm.persistencia.CongregacaoDao;
import java.sql.SQLException;
import java.util.logging.Level;
import java.util.logging.Logger;
import javax.servlet.http.HttpServletRequest;

/**
 *
 * @author viviano
 */
public class ApagarCongregacao implements Command{

    @Override
    public String execute(HttpServletRequest request) {
        String nextPage = "cadastrodecongregacoes.jsp";
        Integer idCongregacao = new Integer(request.getParameter("id_congregacao"));
        
        CongregacaoDao congregacaoDao = new CongregacaoDao();
        try {
            congregacaoDao.apagaCongregacao(idCongregacao);
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(ApagarCongregacao.class.getName()).log(Level.SEVERE, null, ex);
        } catch (SQLException ex) {
            Logger.getLogger(ApagarCongregacao.class.getName()).log(Level.SEVERE, null, ex);
        }
        return nextPage;
    }
    
}
